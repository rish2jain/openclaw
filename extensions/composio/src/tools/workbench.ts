...
export const ComposioWorkbenchSchema = Type.Object({
  ...
  current_step: Type.Optional(
    Type.String({
      description:
        "Short enum for current workflow step (e.g., FETCHING_EMAILS, GENERATING_REPLIES)",
    }),
  ),
...
export function createComposioWorkbenchTool(client: ComposioClient, _config: ComposioConfig) {
  return {
    ...
    async execute(_toolCallId: string, params: Record<string, unknown>) {
      ...
      const currentStepMetric =
        typeof params.current_step_metric === "string" ? params.current_step_metric : undefined;
