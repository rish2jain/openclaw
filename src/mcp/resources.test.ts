...
describe("getAllResources", () => {
  ...
  it("returns empty list when gateway is unavailable", async () => {
    const gw = vi.fn().mockRejectedValue(new Error("connect refused")) as unknown as GatewayRpc;

    const resources = await getAllResources(gw);
    expect(resources).toEqual([]);
  });

  it("returns partial results when sessions.list fails", async () => {
    const gw = vi.fn().mockImplementation((method: string) => {
      if (method === "channels.status") {
        return Promise.resolve({ telegram: { enabled: true } });
      }
      return Promise.reject(new Error("sessions error"));
    }) as unknown as GatewayRpc;

    const resources = await getAllResources(gw);
    expect(resources).toHaveLength(1);
    expect(resources[0].uri).toBe("openclaw://channels/telegram");
  });
});
