...
import type { NetworkPerson, RelationshipEdge } from "../../career/network/types.js";
  ...
  return {
    ...
    async execute(args) {
      try {
        ...
        // Array fields need JSON parsing
  ...
  return {
    ...
    async execute(args) {
      try {
        ...
        // Search all statuses for the listing
  ...
  return {
    ...
    async execute(args) {
      try {
        ...
        let persons: NetworkPerson[] = Array.from(ctx.networkGraph.persons.values());

        if (company) {
          const companyPersons = ctx.networkGraph.getPersonsByCompany(company);
          const ids = new Set(companyPersons.map((p: NetworkPerson) => p.id));
          persons = persons.filter((p: NetworkPerson) => ids.has(p.id));
        }

        if (titleKeywords) {
          const kws = titleKeywords.toLowerCase().split(/\s+/);
          persons = persons.filter(
            (p: NetworkPerson) => p.title && kws.some((kw) => p.title!.toLowerCase().includes(kw)),
          );
        }

        if (tagsArg) {
          const filterTags = tagsArg.split(",").map((t) => t.trim().toLowerCase());
          persons = persons.filter((p: NetworkPerson) =>
            filterTags.some((ft: string) => p.tags.some((pt: string) => pt.toLowerCase() === ft)),
          );
        }
  ...
  return {
    ...
    async execute(args) {
      try {
        ...
        // Validate: need either targetCompany or targetPersonId
        const targetCompanyOrPersonId = targetPersonId ?? targetCompany;
        if (!targetCompanyOrPersonId) {
          return argErrorResult(
            new ArgError("Either 'targetCompany' or 'targetPersonId' is required"),
          );
        }
        const maxHops = maxHopsStr ? parseInt(maxHopsStr, 10) : 3;
        if (isNaN(maxHops) || maxHops < 1 || maxHops > 6) {
          return argErrorResult(new ArgError("'maxHops' must be a number between 1 and 6"));
        }

        // Use the first person in the graph as "self" — convention: person with tag "self"
        const persons: NetworkPerson[] = Array.from(ctx.networkGraph.persons.values());
        const self = persons.find((p: NetworkPerson) => p.tags.includes("self"));
        if (!self) {
          return textResult({
        ...
        const paths = findIntroPaths(ctx.networkGraph, self.id, targetCompanyOrPersonId, maxHops);
  ...
  return {
    ...
    async execute(args) {
      try {
        ...
        // Gather context for LLM drafting
        ...
        const edges: RelationshipEdge[] = ctx.networkGraph.edges.filter(
          (e: RelationshipEdge) => e.fromId === personId || e.toId === personId,
        );
        ...
        const context: Record<string, unknown> = {
          ...
          sharedHistory: edges.flatMap((e: RelationshipEdge) => e.sharedHistory),
          connectionStrength:
            edges.length > 0
              ? Math.max(...edges.map((e: RelationshipEdge) => e.connectionStrength))
              : 0,
        };

        // Attach job context if a listing is referenced
        if (targetListingId) {
          const allListings = ctx.jobStore.getHighMatches(0);
...
function getDefsMap(): Map<string, McpToolDefinition> {
  if (!_defs) {
    ...
    for (const def of getCareerToolDefinitions()) {
      // ToolDefinition and McpToolDefinition are structurally compatible
