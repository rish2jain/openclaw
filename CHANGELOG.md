...
- Session maintenance: default `session.maintenance.mode` is now `enforce` (was `warn`). Session transcripts are automatically pruned after 30 days and capped at 500 entries when config omits `session.maintenance`. Set `session.maintenance.mode: "warn"` to restore previous warn-only behavior. Addresses token/disk growth feedback (Discussion #1949).
